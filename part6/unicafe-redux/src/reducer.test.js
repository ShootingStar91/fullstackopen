...
describe('unicafe reducer', () => {
  ...
  test('reset button works', () => {
    ...
    newState = counterReducer(state, goodAction)
    newState = counterReducer(state, okAction)
    newState = counterReducer(state, badAction)
    newState = counterReducer(state, resetAction)
    expect(newState).toEqual({
      good: 0,
      ok: 0,
      bad: 0
